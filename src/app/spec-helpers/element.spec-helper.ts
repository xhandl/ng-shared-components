...
/**
 * Set option (value) of mat-select identified by `data-testid` attribute.
 *
 * @param fixture
 * @param testId
 * @param value
 */
export function setMatSelectOptionValue<T>(fixture: ComponentFixture<T>, testId: string, value: string): void {
  // Open select
  const select = findEl(fixture, testId);
  select.nativeElement.click();
  fixture.detectChanges();
  // Select option
  const selectOptions = findComponents(fixture, 'mat-option');
  const selectOption = selectOptions.find((option) => option.nativeElement.getAttribute('ng-reflect-value') == value);
  selectOption?.nativeElement?.click();
}

/**
 * Set option at index of mat-select identified by `data-testid` attribute.
 *
 * @param fixture
 * @param testId
 * @param index
 */
export function setMatSelectOptionAt<T>(fixture: ComponentFixture<T>, testId: string, index: number): void {
  // Open select
  const select = findEl(fixture, testId);
  select.nativeElement.click();
  fixture.detectChanges();
